// PLZ → Koordinaten (Auswahl häufiger deutscher PLZ-Bereiche)
// Format: PLZ-Präfix (2-3 Stellen) → [lat, lng]
const PLZ_COORDS = {
...
function getCoords(plz) {
  ...
  // Versuche 3-stellig, dann 2-stellig
  const k3 = p.substring(0,3);
  const k2 = p.substring(0,2);
  return PLZ_COORDS[k3] || PLZ_COORDS[k2] || null;
}
...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (zielCoords) {
      ...
      kmEigenfahrt = Math.round(luftlinie * 1.3); // Umwegfaktor 1.3
    }

    // Cargo International API
